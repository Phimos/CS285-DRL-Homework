...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        obs, acs = ptu.from_numpy(obs), ptu.from_numpy(acs)
        prediction, _ = self(obs, acs, self.obs_mean, self.obs_std, self.acs_mean, self.acs_std, self.delta_mean, self.delta_std)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        target_unnormalized = next_observations - observations
        target = (target_unnormalized - data_statistics['delta_mean']) / (data_statistics['delta_std'] + 1e-8)
        ...
        self.update_statistics(**data_statistics)
        _, delta_pred_normalized = self(observations, actions, self.obs_mean, self.obs_std, self.acs_mean, self.acs_std, self.delta_mean, self.delta_std)
